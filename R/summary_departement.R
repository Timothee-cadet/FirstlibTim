...
summary.departement <- function(x) {
  ...
  if (!inherits(x, "département") & !is.data.frame(x)) {
    stop("L'objet doit être de type 'département' ou un data.frame contenant les informations des élus.")
  }
  ...
  if (!inherits(x$Date.de.naissance, "Date")) {
  ...
  # Commune avec la moyenne d’âge la plus faible et la plus élevée
